Feature: test
  ...
  Scenario: test
    ...
    """
    And 执行请求
    Given 数据库中获取数据设置到全局变量中, sql select * from users where id = ${test1.request.body.d} ,获取行数 0, 获取的参数 id,password,permission
    Then 最近一次请求响应状态是否是 200
    Given 查看全局变量
    Then 从全局变量中取出字段 test1.response.body.g[0].cc 的值,是否等于 wwww
    ...
    And 执行请求
    Given 查看全局变量
    Then responseBody中的 ccc[0].acd 是否等于 aaaa
    Then responseBody中的 bbb 是否等于 1
